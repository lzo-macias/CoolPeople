...
const dataDir = path.resolve(__dirname, '..', '..', '..', 'datatheactualfinal3', 'stanceData');
...
async function summarizeCandidate(name, fullText) {
  ...
  const formattedText = fullText.replace(/([AS]\d{5})/g, '\n$1');

  const issueKeywordHits = scoreByIssue(formattedText);
  const highlights = extractHighlights(formattedText);
...
if (isMain) {
    ...
    const filePath = path.resolve(__dirname, '..', '..', '..', '..', `data/stanceData3/${candidateSlug}.txt`);

    try {
        const data = await readFile(filePath, 'utf-8'); // 📖 wait for file reading

        const summary = await summarizeCandidate(candidateName, data); // 🧠 call OpenAI scoring
        console.log(JSON.stringify(summary, null, 2));

        await writeFile(jsonPath, JSON.stringify(summary, null, 2)); // 💾 save JSON properly
...
//     const summary = await summarizeCandidate(candidateName, data);
//     console.log(JSON.stringify(summary, null, 2));
