...
async def search_bing(candidate_name, page):
    ...
    return list(set([await el.get_attribute('href') for el in elements if await el.get_attribute('href')]))
...
async def scrape_candidate(candidate_name):
    ...
    OUTPUT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../data/textfiles'))
...
if __name__ == "__main__":
    ...
    for candidate in candidates:
        ...
        try:
            asyncio.run(asyncio.wait_for(scrape_candidate(name), timeout=300))
        except asyncio.TimeoutError:
            print(f"⏱️ Timeout while processing {name}, skipping.")
